...
export class UserRepository implements IRepository<User> {
  ...
  async save(input: User): Promise<User> {
    const output = await this.connection.query(`INSERT INTO inventorydb.public.user (id, name, email, password) VALUES ('${input.id}', '${input.name}', '${input.email}', '${input.password}') RETURNING id, name, email`);
